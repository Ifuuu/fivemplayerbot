...
const token = 'ODIyOTU2NDg3NTQ1OTc4OTIw.GaAUO3.kOJGiSJd6bI479akM3jbHjTugtw4d9tkO9t9Zg'; //Dette er discord bottens token som den skal bruge til at logge ind med
...
fs.readdir("./commands/", (err, files) => { // læser directory omkring commands
    ...
    jsfile.forEach((f, i) => { // tager hver java fil
        ...
        bot.commands.set(props.help, props); //tillader brug af kommandoer fra ekstern mappe
    });
});
...
bot.on('ready',() =>{ // Når botten er klar skal den
    console.log(`${bot.user.username} is Online.`) //Skriver i console den er klar
...
function activity(){ // laver funktionen activity
    setTimeout(() => { // laver et loop
        fivereborn.query("202.83.122.246","2626", (err, data) => { // starter fivereborn event, IP:PORT
            if (err) { // logger hvis der er fejl
                console.log(err); // logger fejlen
            } else { //  ellers skal den sætte activity
                bot.user.setActivity(`${data.clients}/${data.maxclients} WARGA`, { type: "PLAYING" }); // sætter activity til spillere inde / max spillere i byen
            }
        });
        activity(); 
    }, 1000);
